...
void RunPerft(std::size_t depth, Position &position, std::optional<Move> current_move,
              std::map<Move, std::size_t> &move_counts) {
    ...
    for (const Move &move: moves) {
        ...
        if (!position.GetCheckers(side)) {
            RunPerft(depth - 1, position, move, move_counts);
        }
    }
}
...
std::map<Move, std::size_t> RunPerft(std::size_t depth, Position &position) {
    std::map<Move, std::size_t> move_counts;
    RunPerft(depth, position, std::nullopt, move_counts);
    return move_counts;
}
