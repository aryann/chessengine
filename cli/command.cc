...
CommandDispatcher &CommandDispatcher::Add(std::string arg, std::unique_ptr<Command> command) {
    command_map_[std::move(arg)] = std::move(command);
    return *this;
}

CommandDispatcher &CommandDispatcher::Add(std::string arg, CommandDispatcher dispatcher) {
