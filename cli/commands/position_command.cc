...
std::expected<void, std::string> ApplyMoves(const std::vector<std::string_view> &uci_moves, Position &position) {
    if (uci_moves.empty()) {
        return {};
    }
    if (uci_moves.front() != "moves") {
        return std::unexpected(std::format("Invalid remainder for position command: {}", uci_moves));
    }
    ...
    for (int i = 1; i < uci_moves.size(); ++i) {
        ...
        position.Do(*move);
        if (position.GetCheckers(~position.SideToMove())) {
            return std::unexpected(std::format("Illegal move (cannot place own king in check): {}", *move));
        }
    }
...
std::expected<void, std::string> StartPos::Run(std::vector<std::string_view> args) {
